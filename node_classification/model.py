...
# GINConv is added to the imports
from torch_geometric.nn import GATConv, GCNConv, SAGEConv, GINConv


class MPNNs(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels,
                 local_layers=3, dropout=0.5, heads=1,
                 pre_ln=False, pre_linear=False, res=False,
                 ln=False, bn=False, jk=False, gnn='gcn'):
        ...
        if not self.pre_linear:
            ...
            # --- GIN Architecture Added ---
            elif gnn == 'gin':
                # GINConv requires a neural network for its update function
                nn1 = torch.nn.Sequential(
                    torch.nn.Linear(in_channels, hidden_channels),
                    torch.nn.ReLU(),
                    torch.nn.Linear(hidden_channels, hidden_channels),
                )
                self.local_convs.append(GINConv(nn1, eps=0., train_eps=True))
            # -----------------------------
            else: # Default to GCN
        ...
        for _ in range(local_layers):
            ...
            # --- GIN Architecture Added ---
                ...
                self.local_convs.append(GINConv(nn, eps=0., train_eps=True))
            # -----------------------------
            else: # Default to GCN
    ...
    def forward(self, x, edge_index):
        # embeddings first
        x_final = self.get_embeddings(x, edge_index)
        # then classifier
